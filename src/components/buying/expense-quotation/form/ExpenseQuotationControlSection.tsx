...
import { api } from '@/api';
import { BankAccount, Currency, DuplicateExpensQuotationDto, Invoice, EXPENSQUOTATION_STATUS } from '@/types';
...
export const ExpenseQuotationControlSection = ({
  ...
  const { mutate: removeQuotation, isPending: isDeletePending } = useMutation({
    ...
    onSuccess: () => {
      ...
      router.push('/buying/expense-quotations');
    },
    onError: (error) => {
  ...
  id={quotationManager?.id || 0}
  status={quotationManager?.status as unknown as EXPENSQUOTATION_STATUS} // Explicit cast
  sequential={sequential}
  open={invoiceDialog}
...
/>
      ...
      <div className={cn(className)}>
        ...
        {/* Invoice list */}
        {status === EXPENSQUOTATION_STATUS.Invoiced && invoices.length != 0 && (
          <ExpenseQuotationInvoiceList className="border-b" invoices={invoices} />
        )}
