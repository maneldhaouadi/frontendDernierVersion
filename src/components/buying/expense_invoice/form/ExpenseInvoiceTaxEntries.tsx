...
interface ExpenseInvoiceTaxEntriesProps {
  className?: string;
...
export const ExpenseInvoiceTaxEntries: React.FC<ExpenseInvoiceTaxEntriesProps> = ({
  className,
  article,
  taxes,
  selectedTaxIds,
  ...
  return (
    <div className={cn('flex flex-col', className)}>
      {(!article?.expenseArticleInvoiceEntryTaxes || article?.expenseArticleInvoiceEntryTaxes?.length == 0) && (
        <p className="font-thin text-sm m-1">{tInvoicing('article.no_applied_tax')}</p>
      )}
      {article.expenseArticleInvoiceEntryTaxes?.map((appliedTax, i) => (
